//! Named pipe server: `\\.\pipe\instant-file-search-indexer` (Windows).
//!
//! Protocol: one JSON request per connection, one JSON response, then close.
//! The protocol logic itself lives in `protocol` (portable); this module is
//! only the Windows transport (named pipe creation + read/write).
